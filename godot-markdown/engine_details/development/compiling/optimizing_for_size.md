...
Godot features an `doc_engine_compilation_configuration_editor` tool that can detect the features used in the current project and create a build profile. Once saved, this build profile can then be passed to SCons when compiling custom export templates:
...
- `class_MenuButton`
- `class_OptionButton`
- `class_PopupMenu` (will make all popup menus unavailable in code for classes that use them, like `class_LineEdit`, even though those classes are still available)
