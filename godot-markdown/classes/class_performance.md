...
`void (No return value.)` **add_custom_monitor**(id: `StringName<class_StringName>`, callable: `Callable<class_Callable>`, arguments: `Array<class_Array>` = \[\], type: `MonitorType<enum_Performance_MonitorType>` = 0) `🔗<class_Performance_method_add_custom_monitor>`
...
\# Adds monitor with name "MyName" to category "MyCategory". Performance.add_custom_monitor("MyCategory/MyMonitor", monitor_value)

\# Adds monitor with name "MyName" to category "Custom". \# Note: "MyCategory/MyMonitor" and "MyMonitor" have same name but different IDs, so the code is valid. Performance.add_custom_monitor("MyMonitor", monitor_value)

\# Adds monitor with name "MyName" to category "Custom". \# Note: "MyMonitor" and "Custom/MyMonitor" have same name and same category but different IDs, so the code is valid. Performance.add_custom_monitor("Custom/MyMonitor", monitor_value)
...
public override void Ready() { var monitorValue = new Callable(this, MethodName.GetMonitorValue);
...
`float<class_float>` **get_monitor**(monitor: `Monitor<enum_Performance_Monitor>`) `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_Performance_method_get_monitor>`
...
`void (No return value.)` **remove_custom_monitor**(id: `StringName<class_StringName>`) `🔗<class_Performance_method_remove_custom_monitor>`
