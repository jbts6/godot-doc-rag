...
`PackedStringArray<class_PackedStringArray>` **\_get_build_dependencies**(path: `String<class_String>`) `virtual (This method should typically be overridden by the user to have any effect.)` `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_ResourceImporter_private_method__get_build_dependencies>`
