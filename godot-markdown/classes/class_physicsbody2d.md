...
`void (No return value.)` **add_collision_exception_with**(body: `Node<class_Node>`) `🔗<class_PhysicsBody2D_method_add_collision_exception_with>`
...
`Vector2<class_Vector2>` **get_gravity**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_PhysicsBody2D_method_get_gravity>`
...
`void (No return value.)` **remove_collision_exception_with**(body: `Node<class_Node>`) `🔗<class_PhysicsBody2D_method_remove_collision_exception_with>`
