...
- `void (No return value.)` **set_action_mode**(value: `ActionMode<enum_BaseButton_ActionMode>`)
...
- `void (No return value.)` **set_button_group**(value: `ButtonGroup<class_ButtonGroup>`)
...
`BitField (This value is an integer composed as a bitmask of the following flags.)`\[`MouseButtonMask<enum_@GlobalScope_MouseButtonMask>`\] **button_mask** = `1` `🔗<class_BaseButton_property_button_mask>`
...
- `void (No return value.)` **set_keep_pressed_outside**(value: `bool<class_bool>`)
...
- `void (No return value.)` **set_shortcut**(value: `Shortcut<class_Shortcut>`)
...
- `void (No return value.)` **set_shortcut_feedback**(value: `bool<class_bool>`)
...
- `void (No return value.)` **set_shortcut_in_tooltip**(value: `bool<class_bool>`)
- `bool<class_bool>` **is_shortcut_in_tooltip_enabled**()

If `true`, the button will add information about its shortcut in the tooltip. This includes the shortcut's events and its `Resource.resource_name<class_Resource_property_resource_name>`. If both events and name are empty, the shortcut will not be included.
...
`void (No return value.)` **\_pressed**() `virtual (This method should typically be overridden by the user to have any effect.)` `🔗<class_BaseButton_private_method__pressed>`
...
`void (No return value.)` **\_toggled**(toggled_on: `bool<class_bool>`) `virtual (This method should typically be overridden by the user to have any effect.)` `🔗<class_BaseButton_private_method__toggled>`
...
`DrawMode<enum_BaseButton_DrawMode>` **get_draw_mode**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_BaseButton_method_get_draw_mode>`

Returns the visual state used to draw the button. This is useful mainly when implementing your own draw code by either overriding draw() or connecting to "draw" signal. The visual state of the button is defined by the `DrawMode<enum_BaseButton_DrawMode>` enum.

classref-item-separator

classref-method

`bool<class_bool>` **is_hovered**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_BaseButton_method_is_hovered>`
...
`void (No return value.)` **set_pressed_no_signal**(pressed: `bool<class_bool>`) `🔗<class_BaseButton_method_set_pressed_no_signal>`
