...
| **sampler2D** | hint_roughness\[r, g, b, a, normal, gray\] | Used for roughness limiter on import (attempts reducing specular aliasing). `_normal` is a normal map that guides the roughness limiter, with roughness increasing in areas that have high-frequency detail. |
| **sampler2D** | filter\[nearest, linear\]\[mipmap\]\[anisotropic\] | Enabled specified texture filtering. |
| **sampler2D** | repeat\[enable, disable\] | Enabled texture repeating. |
...
<pre class="sourceCode gdscript"><code class="sourceCode gdscript"><a href="#cb1-1" aria-hidden="true" tabindex="-1"></a>bvec2_input: int = (int(bx)) | (int(by) &lt;&lt; 1)</code></pre></td>
</tr>
<tr>
