...
>
> public override void Ready() { Instance = this; }
...
func ready():
var root = get_tree().root \# Using a negative index counts from the end, so this gets the last child node of root. current_scene = root.get_child(-1)
...
> public override void Ready() { Viewport root = GetTree().Root; // Using a negative index counts from the end, so this gets the last child node of root. CurrentScene = root.GetChild(-1); }
