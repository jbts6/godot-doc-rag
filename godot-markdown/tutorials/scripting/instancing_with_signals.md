...
> public override void PhysicsProcess(double delta) { Position += Velocity \* (float)delta; }
...
>
> public override void Input(InputEvent @event) { if (@event is InputEventMouseButton mouseButton) { if (mouseButton.ButtonIndex == MouseButton.Left && mouseButton.Pressed) { EmitSignal(SignalName.Shoot, bullet, Rotation, Position); } } }
>
> public override void Process(double delta) { LookAt(GetGlobalMousePosition()); }

}

In the main scene, we then connect the player's signal (it will appear in the "Node" tab of the Inspector)

func on_player_shoot(Bullet, direction, location):
