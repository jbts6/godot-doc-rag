...
public partial class MyMeshInstance3D : MeshInstance3D { public override void Ready() { Godot.Collections.Array surfaceArray = \[\]; surfaceArray.Resize((int)Mesh.ArrayType.Max);
...
public partial class MeshInstance3d : MeshInstance3D { public override void Ready() { // Insert setting up the surface array and lists here.
...
public partial class MyMeshInstance3D : MeshInstance3D { private int rings = 50; private int radialSegments = 50; private float radius = 1;

> public override void Ready() { // Insert setting up the surface array and lists here.
>
> > // Vertex indices. var thisRow = 0; var prevRow = 0; var point = 0;
> >
> > // Loop over rings. for (var i = 0; i \< rings + 1; i++) { var v = ((float)i) / rings; var w = Mathf.Sin(Mathf.Pi \* v); var y = Mathf.Cos(Mathf.Pi \* v);
> >
> > > // Loop over segments in ring. for (var j = 0; j \< radialSegments + 1; j++) { var u = ((float)j) / radialSegments; var x = Mathf.Sin(u \* Mathf.Pi \* 2); var z = Mathf.Cos(u \* Mathf.Pi \* 2); var vert = new Vector3(x \* radius \* w, y \* radius, z \* radius \* w); verts.Add(vert); normals.Add(vert.Normalized()); uvs.Add(new Vector2(u, v)); point += 1;
> > >
