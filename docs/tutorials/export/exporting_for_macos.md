...
> [!NOTE]
> A valid bundle ID can only contain alphanumeric characters, hyphens, and periods (`A-Z`, `a-z`, `0-9`, `-`, and `.`). Apple recommends using reverse-DNS format (e.g. `com.example.your-game`) of a domain you own, so that your bundle ID is guaranteed to be unique. Bundle IDs are case-insensitive. See [CFBundleIdentifier](https://developer.apple.com/documentation/bundleresources/information-property-list/cfbundleidentifier).
...
| Address Book | [^10] Enable to allow access contacts in the user's address book, if it's enabled you should also provide usage message in the privacy/address_book_usage_description option. |
| Calendars | [^11] Enable to allow access to the user's calendar, if it's enabled you should also provide usage message in the privacy/calendar_usage_description option. |
| Photo Library | [^12] Enable to allow access to the user's Photos library, if it's enabled you should also provide usage message in the privacy/photos_library_usage_description option. |
...
[^15]: You can optionally provide usage messages for various folders in the privacy/\*\_folder_usage_description options.

[^16]: You can optionally provide usage messages for various folders in the privacy/\*\_folder_usage_description options.

[^17]: You can optionally provide usage messages for various folders in the privacy/\*\_folder_usage_description options.

[^18]: You can optionally provide usage messages for various folders in the privacy/\*\_folder_usage_description options.

[^19]: You can optionally provide usage messages for various folders in the privacy/\*\_folder_usage_description options.