...
func ready():
\# Use call deferred to make sure the entire scene tree nodes are setup \# else await on 'physics_frame' in a ready() might get stuck. custom_setup.call_deferred()
...
public partial class MyNode3D : Node3D { public override void Ready() { // Use call deferred to make sure the entire scene tree nodes are setup // else await on 'physics_frame' in a Ready() might get stuck. CallDeferred(MethodName.CustomSetup); }
