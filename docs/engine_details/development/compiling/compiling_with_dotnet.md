...
> [!NOTE]
> C# support for Godot has historically used the [Mono](https://www.mono-project.com/) runtime instead of the [.NET Runtime](https://github.com/dotnet/runtime) and internally many things are still named `mono` instead of `dotnet` or otherwise referred to as `mono`.
