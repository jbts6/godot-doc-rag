...
[tests/test_macros.h](https://github.com/godotengine/godot/blob/master/tests/test_macros.h) source code for currently implemented macros and aliases for them.
...
Godot supports writing tests per C++ module. For instructions on how to write module tests, refer to `doc_custom_module_unit_tests`.
...
The custom command-line parsing can be performed by a test tool itself with the help of OS `get_cmdline_args<class_OS_method_get_cmdline_args>` method.
...
> [!NOTE]
> The GDScript test runner is meant for testing the GDScript implementation, not for testing user scripts nor testing the engine using scripts. We recommend writing new tests for already resolved [issues related to GDScript at GitHub](https://github.com/godotengine/godot/issues?q=is%3Aissue+label%3Atopic%3Agdscript+is%3Aclosed), or writing tests for currently working features.
