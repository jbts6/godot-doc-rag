...
`void (No return value.)` **disconnect_from_peer**() `🔗<class_PacketPeerDTLS_method_disconnect_from_peer>`
...
`Status<enum_PacketPeerDTLS_Status>` **get_status**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_PacketPeerDTLS_method_get_status>`
...
`void (No return value.)` **poll**() `🔗<class_PacketPeerDTLS_method_poll>`
