...
- `void (No return value.)` **set_anchor_detection_is_enabled**(value: `bool<class_bool>`)
...
- `void (No return value.)` **set_primary**(value: `bool<class_bool>`)
...
`int<class_int>` **get_capabilities**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_XRInterface_method_get_capabilities>`
...
`StringName<class_StringName>` **get_name**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_XRInterface_method_get_name>`
...
`PackedVector3Array<class_PackedVector3Array>` **get_play_area**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_XRInterface_method_get_play_area>`
...
`TrackingStatus<enum_XRInterface_TrackingStatus>` **get_tracking_status**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_XRInterface_method_get_tracking_status>`
...
`bool<class_bool>` **is_initialized**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_XRInterface_method_is_initialized>`
...
`void (No return value.)` **stop_passthrough**() `🔗<class_XRInterface_method_stop_passthrough>`
...
`void (No return value.)` **trigger_haptic_pulse**(action_name: `String<class_String>`, tracker_name: `StringName<class_StringName>`, frequency: `float<class_float>`, amplitude: `float<class_float>`, duration_sec: `float<class_float>`, delay_sec: `float<class_float>`) `🔗<class_XRInterface_method_trigger_haptic_pulse>`
...
`void (No return value.)` **uninitialize**() `🔗<class_XRInterface_method_uninitialize>`
