...
`void (No return value.)` **clear_cache**(hostname: `String<class_String>` = "") `🔗<class_IP_method_clear_cache>`
...
`void (No return value.)` **erase_resolve_item**(id: `int<class_int>`) `🔗<class_IP_method_erase_resolve_item>`
...
`PackedStringArray<class_PackedStringArray>` **get_local_addresses**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_IP_method_get_local_addresses>`
...
`Array<class_Array>`\[`Dictionary<class_Dictionary>`\] **get_local_interfaces**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_IP_method_get_local_interfaces>`
...
`String<class_String>` **get_resolve_item_address**(id: `int<class_int>`) `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_IP_method_get_resolve_item_address>`
...
`Array<class_Array>` **get_resolve_item_addresses**(id: `int<class_int>`) `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_IP_method_get_resolve_item_addresses>`
...
`ResolverStatus<enum_IP_ResolverStatus>` **get_resolve_item_status**(id: `int<class_int>`) `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_IP_method_get_resolve_item_status>`
