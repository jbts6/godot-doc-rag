...
`void (No return value.)` **cancel_future**(future: `int<class_int>`) `🔗<class_OpenXRFutureExtension_method_cancel_future>`
...
`bool<class_bool>` **is_active**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_OpenXRFutureExtension_method_is_active>`
