...
Alternatively, you can use [this tool](https://danilw.github.io/GLSL-howto/cubemap_to_panorama_js/cubemap_to_panorama.html) to convert a cubemap to an equirectangular sky map and use `PanoramaSkyMaterial<class_PanoramaSkyMaterial>` as usual.
...
`Resource<class_Resource>` **create_placeholder**() `const (This method has no side effects. It doesn't modify any of the instance's member variables.)` `🔗<class_Cubemap_method_create_placeholder>`
