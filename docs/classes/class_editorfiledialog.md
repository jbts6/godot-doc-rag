...
**EditorFileDialog** will automatically show a native dialog based on the `EditorSettings.interface/editor/appearance/use_native_file_dialogs<class_EditorSettings_property_interface/editor/appearance/use_native_file_dialogs>` editor setting and ignores `FileDialog.use_native_dialog<class_FileDialog_property_use_native_dialog>`.
...
- `void (No return value.)` **set_disable_overwrite_warning**(value: `bool<class_bool>`)
...
`void (No return value.)` **add_side_menu**(menu: `Control<class_Control>`, title: `String<class_String>` = "") `🔗<class_EditorFileDialog_method_add_side_menu>`
