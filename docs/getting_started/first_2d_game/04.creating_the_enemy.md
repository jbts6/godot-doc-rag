...
Select the `Mob` node and set its `Gravity Scale` property in the `RigidBody2D <class_RigidBody2D>` section of the inspector to `0`. This will prevent the mob from falling downwards.

In addition, in the `Mob` node inspector, under the `CollisionObject2D <class_CollisionObject2D>` section, expand the **Collision** group and uncheck the `1` inside the `Mask` property. This will ensure the mobs do not collide with each other.

![image](img/set_collision_mask.webp)

Set up the `AnimatedSprite2D <class_AnimatedSprite2D>` like you did for the player. This time, we have 3 animations: `fly`, `swim`, and `walk`. There are two images for each animation in the art folder.
...
func ready():
var mob_types = Array(\$AnimatedSprite2D.sprite_frames.get_animation_names()) \$AnimatedSprite2D.animation = mob_types.pick_random() \$AnimatedSprite2D.play()

csharp

public override void Ready() { var animatedSprite2D = GetNode\<AnimatedSprite2D\>("AnimatedSprite2D"); string\[\] mobTypes = animatedSprite2D.SpriteFrames.GetAnimationNames(); animatedSprite2D.Play(mobTypes\[GD.Randi() % mobTypes.Length\]); }
